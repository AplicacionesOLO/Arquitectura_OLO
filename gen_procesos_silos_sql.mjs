...
//   de eFlow WMS llevan adjunta su captura (bucket Detalles_Porcesos/wms-manual/).
...
const { PROCESOS_SILOS } = await import(pathToFileURL(`${APP}/data/procesos_silos.js`).href);
...
for (const [codigo, p] of Object.entries(PROCESOS_SILOS)) {
  ...
  if v_macro is null then raise exception 'No existe el macroproceso % en %', ${q(p.macro)}, ${q(p.silo)}; end if;
  ...
  p.pasos.forEach((st, i) => {
    ...
    const w = st.screen && WMS_INDEX[st.screen];
    if (w?.img) {
      const size = fs.statSync(`${IMG_DIR}/${w.img}`).size;
...
const imgs = n.reduce((s, p) => s + p.pasos.filter(x => x.screen && WMS_INDEX[x.screen]?.img).length, 0);
