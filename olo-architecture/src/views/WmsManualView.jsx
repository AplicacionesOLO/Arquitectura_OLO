...
import { SelectorRecorrido } from "../components/SelectorRecorrido.jsx";
...
export function WmsManualView({ focus }) {
  ...
  // En pantallas angostas con el panel abierto, el menú de módulos pasa a chips
  // arriba para dejar espacio a las tarjetas.
  const compacto = ancho < 1500 && !!sel;
  const recorridos = [...PROCESOS_CEDI_ORDEN, ...Object.keys(PROCESOS).filter(c => !PROCESOS_CEDI_ORDEN.includes(c))].filter(c => PASO_PANTALLA[c]);

  return <div style={{ display:"flex", gap:16, alignItems:"flex-start" }}>
    {!compacto && <nav style={{ width:210, minWidth:210, background:"#fff", border:`1px solid ${DESIGN.border}`, borderRadius:10, overflow:"hidden", flexShrink:0, position:"sticky", top:20, maxHeight:"calc(100vh - 40px)", overflowY:"auto" }}>
      <div style={{ padding:"10px 14px", borderBottom:`1px solid ${DESIGN.sunken2}`, background:DESIGN.sunken, fontSize:12, fontWeight:700, color:DESIGN.muted, letterSpacing:"0.08em", textTransform:"uppercase" }}>eFlow WMS {data.version} · módulos</div>
      {data.modules.map(m => {
        ...
        return <button key={m.name} onClick={()=>{ setModName(m.name); setQ(""); }} style={{ display:"flex", alignItems:"center", gap:8, width:"100%", padding:"9px 14px", border:"none", borderLeft:`3px solid ${isA?ACCENT:"transparent"}`, borderBottom:`1px solid ${DESIGN.sunken}`, background:isA?ACCENT+"12":"transparent", cursor:"pointer", fontFamily:"inherit", textAlign:"left" }}>
          <span style={{ flex:1, fontSize:14, fontWeight:isA?700:500, color:isA?DESIGN.ink:DESIGN.inkSoft }}>{m.name}</span>
          {usados > 0 && <span title="Pantallas usadas en procesos" style={{ fontSize:11.5, fontWeight:700, color:"#16a34a" }}>●{usados}</span>}
          <span style={{ fontSize:12, color:DESIGN.mutedSoft }}>{m.screens.length}</span>
        </button>;
      })}
      <div style={{ padding:"10px 14px", fontSize:12.5, color:DESIGN.mutedSoft, lineHeight:1.5 }}>
        <span style={{ color:"#16a34a", fontWeight:700 }}>●</span> pantallas que usan los procesos
      </div>
    </nav>}
    ...
    <div style={{ flex:1, minWidth:0 }}>
      <div style={{ position:"relative", maxWidth:420, marginBottom:12 }}>
        ...
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar pantalla, campo o columna en todo el WMS…"
          style={{ width:"100%", boxSizing:"border-box", fontSize:14.5, border:`1px solid ${DESIGN.borderStrong}`, borderRadius:8, padding:"8px 12px 8px 32px", fontFamily:DESIGN.font, outline:"none" }}/>
      </div>
      {compacto && <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
        {data.modules.map(m => { const isA = !results && mod.name === m.name;
          return <button key={m.name} onClick={()=>{ setModName(m.name); setQ(""); }} style={{ padding:"5px 12px", borderRadius:DESIGN.radiusPill, border:`1px solid ${isA?DESIGN.ink:DESIGN.border}`, background:isA?DESIGN.ink:"#fff", color:isA?"#fff":DESIGN.inkSoft, fontSize:14.5, cursor:"pointer", fontFamily:DESIGN.font }}>{m.name}</button>; })}
      </div>}
      {!results && <SelectorRecorrido codigos={recorridos} onPlay={(codigo) => setShow({ slides: slidesProceso(codigo), start: 0 })}/>}
      {results
        ? <div style={{ fontSize:14, color:DESIGN.muted, marginBottom:10 }}>{results.length} pantalla{results.length===1?"":"s"} con "{q}"</div>
        : <div style={{ marginBottom:12 }}>
            <div style={{ fontSize:17, fontWeight:700, color:DESIGN.ink }}>Módulo {mod.name}</div>
            <p style={{ fontSize:14, color:DESIGN.inkSoft, lineHeight:1.6, margin:"4px 0 0", maxWidth:820 }}>{mod.info}</p>
          </div>}
...
function ScreenCard({ s, active, onClick, showModule }) {
  ...
  return <button onClick={onClick} style={{ textAlign:"left", background:"#fff", border:`1px solid ${active?DESIGN.ink:DESIGN.border}`, boxShadow:active?`0 0 0 1px ${DESIGN.ink}`:"none", borderRadius:9, padding:0, cursor:"pointer", fontFamily:DESIGN.font, overflow:"hidden", display:"flex", flexDirection:"column" }}>
    {s.img
      ...
      : <div style={{ aspectRatio:"1800/969", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:DESIGN.mutedSoft, background:DESIGN.sunken, borderBottom:`1px solid ${DESIGN.border}` }}>sin captura</div>}
    <div style={{ padding:"8px 10px 10px" }}>
      {showModule && <div style={{ fontSize:12, color:DESIGN.muted }}>{s.module}</div>}
      <div style={{ fontSize:14, fontWeight:700, color:DESIGN.ink }}>{s.option}</div>
      {s.title && s.title !== s.option && <div style={{ fontSize:12.5, color:DESIGN.muted }}>ventana «{s.title}»</div>}
      <div style={{ display:"flex", gap:5, flexWrap:"wrap", marginTop:6 }}>
        {procs.length > 0 && <span style={{ fontSize:11.5, fontWeight:700, color:"#16a34a", background:"#16a34a14", borderRadius:4, padding:"1px 5px" }}>{procs.map(p => p.codigo).join(" · ")}</span>}
        {cols > 0 && <span style={{ fontSize:11.5, color:DESIGN.muted, background:DESIGN.sunken2, borderRadius:4, padding:"1px 5px" }}>{cols} columnas</span>}
        {s.buttons.length > 0 && <span style={{ fontSize:11.5, color:DESIGN.muted, background:DESIGN.sunken2, borderRadius:4, padding:"1px 5px" }}>{s.buttons.length} acciones</span>}
      </div>
    </div>
  </button>;
}
...
function ScreenPanel({ s, byId, onOpen, onClose, onPlay }) {
  ...
  return <aside style={{ width:"clamp(360px, 34vw, 520px)", flexShrink:0, position:"sticky", top:20, maxHeight:"calc(100vh - 40px)", display:"flex", flexDirection:"column", background:"#fff", border:`1px solid ${DESIGN.border}`, borderLeft:`4px solid ${ACCENT}`, borderRadius:10, overflow:"hidden", boxSizing:"border-box" }}>
    <div style={{ padding:"12px 16px 0", flexShrink:0 }}>
      <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:12.5, color:DESIGN.muted }}>{s.module} › {s.option}</div>
          <div style={{ fontSize:16, fontWeight:700, color:DESIGN.ink }}>{s.title || s.option}</div>
          {s.formId && <div style={{ fontSize:12, color:DESIGN.mutedSoft, fontFamily:"'Courier New', monospace" }}>{s.formId}{s.loadSeconds ? ` · carga ${s.loadSeconds}s` : ""}</div>}
        </div>
      ...
      {s.img && <div style={{ position:"relative", marginTop:10, cursor:"zoom-in" }} onClick={()=>onPlay({ slides: vistas, start: 0 })} title="Ver en grande y recorrer la pantalla">
        ...
        <span style={{ position:"absolute", right:8, bottom:8, fontSize:13, fontWeight:700, color:"#fff", background:"rgba(15,23,42,0.8)", borderRadius:6, padding:"3px 8px" }}>
      ...
      <div style={{ display:"flex", gap:2, marginTop:10, borderBottom:`1px solid ${DESIGN.border}` }}>
        {TABS.map(([id,label]) => <button key={id} onClick={()=>setTab(id)} style={{ fontSize:13.5, fontWeight:tab===id?700:500, color:tab===id?DESIGN.ink:DESIGN.muted, background:"none", border:"none", borderBottom:`2px solid ${tab===id?ACCENT:"transparent"}`, padding:"6px 8px", cursor:"pointer", fontFamily:DESIGN.font, marginBottom:-1 }}>{label}</button>)}
      </div>
    </div>
    <div style={{ padding:"12px 16px 16px", overflowY:"auto", flex:1 }}>
      {tab==="conexiones" && <>
          ...
          {procs.map(({ codigo, pasos }) => {
            ...
            return <div key={codigo} style={linkBox}>
              <div style={{ display:"flex", gap:8, alignItems:"baseline" }}>
                <button onClick={()=>navigate({ tab:"olo-arch", codigo })} title="Abrir la ficha del proceso" style={{ ...inlineLink, fontSize:14, color:DESIGN.ink, flex:1, textAlign:"left" }}>{codigo} · {p.nombre} ↗</button>
                {pasos.length > 0 && <button onClick={()=>onPlay({ slides: slidesProceso(codigo), start: pasos[0] - 1 })} title="Presentar el proceso desde este paso"
                  style={{ fontSize:12.5, fontWeight:700, color:"#fff", background:ACCENT, border:"none", borderRadius:5, padding:"2px 8px", cursor:"pointer", fontFamily:DESIGN.font, flexShrink:0 }}>▶ Recorrido desde paso {pasos[0]}</button>}
              </div>
              {pasos.length > 0 && <div style={{ fontSize:13, color:DESIGN.muted, marginTop:2 }}>
        ...
        <div style={{ display:"grid", gap:5 }}>
          {tablas.map(t => <div key={t.tabla} style={{ display:"flex", gap:8, alignItems:"baseline" }}>
            <button onClick={()=>navigate({ tab:"integrations", cat:"efw", table:t.tabla })} title="Ver la tabla en Integraciones" style={{ fontSize:13.5, fontWeight:700, color:ACCENT, background:"none", border:"none", padding:0, cursor:"pointer", fontFamily:"'Courier New', monospace", flexShrink:0 }}>{t.tabla}</button>
            <span style={{ fontSize:12.5, color:DESIGN.muted }}>
              {t.columnas.length > 0 ? `columnas: ${t.columnas.join(", ")}` : `usada por ${t.porProceso} en esta pantalla`}
            </span>
          </div>)}
        </div>
        {tablas.length > 0 && <div style={{ fontSize:12, color:DESIGN.mutedSoft, marginTop:6 }}>Inferidas: columnas del grid que coinciden con columnas clave de la tabla, o tablas que el proceso usa en esta pantalla.</div>}
        {s.nav.length > 0 && <>
          <L n={s.nav.length}>Navegación desde esta pantalla</L>
          <div style={{ display:"grid", gap:4 }}>
            {s.nav.map((n,i) => <div key={i} style={{ fontSize:13.5, color:DESIGN.inkSoft }}>
      ...
      {tab==="pantalla" && <>
        ...
        {vistas.map((sl, k) => <div key={k}>
          ...
          <div style={{ fontSize:12.5, color:DESIGN.muted, marginTop:2 }}>{sl.texto}</div>
        </div>)}
      ...
      {tab==="campos" && <>
        ...
        <div style={{ display:"grid", gap:5 }}>
          {s.buttons.map((b,i) => <div key={i} style={{ display:"flex", gap:8, alignItems:"baseline", fontSize:14 }}>
            <b style={{ color:b.enabled===false?DESIGN.mutedSoft:DESIGN.ink, flexShrink:0 }}>{b.name}</b>
            <span style={{ fontSize:11.5, fontWeight:700, color:(KIND[b.kind]||KIND["estándar"]).color, flexShrink:0 }}>{(KIND[b.kind]||KIND["estándar"]).label}</span>
            {b.desc && <span style={{ color:DESIGN.muted, fontSize:13 }}>{b.desc}</span>}
          </div>)}
        </div>
        {s.menu.length > 0 && <><L>Menú «…»</L>{s.menu.map(g => <div key={g.group} style={{ fontSize:14, color:DESIGN.inkSoft, marginBottom:4 }}><b>{g.group}:</b> {g.items.map(it => it.name).join(" · ")}</div>)}</>}
      </>}
      {tab==="columnas" && (s.tables.length === 0 ? <T>Sin grids en esta pantalla.</T> : s.tables.map((t,i) => <div key={i}>
        ...
        <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
          {t.columns.map(c => <span key={c.name} title={c.hidden ? "Columna oculta por defecto" : undefined} style={{ fontSize:12.5, color:c.hidden?DESIGN.mutedSoft:DESIGN.inkSoft, background:c.hidden?"transparent":DESIGN.sunken2, border:`1px ${c.hidden?"dashed":"solid"} ${DESIGN.border}`, borderRadius:4, padding:"1px 6px" }}>{c.name}</span>)}
        </div>
      </div>))}
    </div>
  </aside>;
...
function Hilo({ procs, s, tablas }) {
  const cell = (label, value, on) => <div style={{ flex:1, minWidth:0, padding:"6px 8px", borderRadius:6, background:on?ACCENT+"14":DESIGN.sunken, border:`1px solid ${on?ACCENT+"55":DESIGN.border}` }}>
    <div style={{ fontSize:11, fontWeight:700, color:DESIGN.muted, textTransform:"uppercase", letterSpacing:"0.05em" }}>{label}</div>
    <div style={{ fontSize:13, fontWeight:700, color:on?DESIGN.ink:DESIGN.inkSoft, lineHeight:1.3, wordBreak:"break-word" }}>{value}</div>
  </div>;
  const arrow = <span style={{ color:DESIGN.mutedSoft, fontSize:13 }}>→</span>;
...
const inlineLink = { background:"none", border:"none", padding:0, color:ACCENT, fontWeight:700, cursor:"pointer", fontFamily:DESIGN.font, fontSize:13.5 };
function L({ children, n }) {
  return <div style={{ fontSize:12, fontWeight:700, color:DESIGN.muted, letterSpacing:"0.07em", textTransform:"uppercase", margin:"14px 0 6px" }}>{children}{n != null && <span style={{ color:DESIGN.mutedSoft, fontWeight:400 }}> · {n}</span>}</div>;
}
function T({ children }) { return <p style={{ fontSize:14, color:DESIGN.muted, margin:0 }}>{children}</p>; }
